...
void dot_product_on_neurons_with_sig2(double neurons[], int n_size,
									  const double input[], double (*weight)[2], int i_size)
{
	for (int i = 0; i < n_size; ++i)
	{
		double product = 0;
...
void predict(double neurons[], double output[], double input[], double (*weight1)[3], double (*weight2)[2])
{
	dot_product_on_neurons_with_sig1(neurons, 2, input, weight1, 3);
	dot_product_on_neurons_with_sig2(output, 3, neurons, weight2, 2);
}

void learn(double (*epoch)[3], double neurons[], double output[], double (*weight1)[3], double (*weight2)[2])
{
	double	step  = 0.5;
	int		iters = 1000;
