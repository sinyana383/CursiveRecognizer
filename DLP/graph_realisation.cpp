...
void GraphPerceptron::genWeights()
{
...
void GraphPerceptron::train(int indexInput)
{
  _expectedLetter = vector_vectorovi4[indexInput][0] - 1;
  for (size_t i = 1; i < vector_vectorovi4[indexInput].size(); i++)
    layers_[0].SetNeuronValue(i, vector_vectorovi4[indexInput][i] / 255);
  ForwardPropagation();
...
void GraphPerceptron::test(int d)
{
